...
import { Globe, LogOut, ShoppingBag } from 'lucide-react';

const Header: React.FC = () => {
  const { language, setLanguage, setRole, setView } = useApp();

  return (
    <header className="bg-white text-[#061E29] border-b border-[#1D546D]/10 sticky top-0 z-50 pt-[env(safe-area-inset-top)] transition-colors duration-500">
      <div className="container mx-auto px-4 h-14 flex items-center justify-between">
        <div 
          className="flex items-center gap-2 cursor-pointer" 
          onClick={() => setView('home')}
        >
          <div className="w-8 h-8 bg-[#061E29] rounded-lg flex items-center justify-center shadow-md">
          ...
          <div className="flex flex-col">
            ...
            <div className="flex items-center gap-1 mt-0.5">
              <div className="w-1 h-1 bg-[#5F9598] rounded-full animate-pulse"></div>
              <span className="text-[7px] font-black uppercase tracking-widest text-[#1D546D]/50">
                Fresh Daily
              </span>
            </div>
          </div>
        ...
        <div className="flex items-center gap-3">
          <div className="relative group">
            <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-full transition-all text-xs font-bold uppercase active:scale-95 bg-[#F3F4F4] text-[#1D546D]">
              <Globe size={14} />
              {language}
            </button>
          ...
          <button 
            ...
            className="p-2 transition-all active:scale-90 text-[#1D546D]/30"
          >
            <LogOut size={20} />
          </button>
        </div>
